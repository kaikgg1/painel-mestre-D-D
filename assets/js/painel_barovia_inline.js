...
// O painel carrega só quem está ativo (DBSync apenasAtivos pro Mestre), então
// um PJ desativado simplesmente desaparecia da tela e o Mestre não tinha
// NENHUM caminho pra trazer de volta — só o dono da ficha, pelo menu ⋯ dela.
// Esta barra lista os inativos da campanha e devolve esse controle pro Mestre.
...
async function renderInativos() {
  ...
  // "Personagem Padrão" é o placeholder que o grimório cria só pra ancorar as
  // magias favoritas — nunca foi um PJ de verdade (mesmo filtro das
...
function criarCard(p) {
  ...
  if (ehConjurador) {
    ...
    function pintarMagias(magias) {
      ...
      // Delegação nos dois containers (a aba "Magias" recebe uma cópia do HTML).
      ...
      [prepWrap, magiasTabContent].forEach(cont => {
        if (!cont) return;
        cont._dcMagias = magias;
        if (cont.dataset.dcLigado) return;
        cont.dataset.dcLigado = '1';
        const abrir = alvo => {
          const m = (cont._dcMagias || [])[+alvo.dataset.magia];
  ...
  let ativoBtn = null;
  if (ehMestre) {
    ativoBtn = document.createElement('button');
  ...
  if (ativoBtn) actions.appendChild(ativoBtn);
